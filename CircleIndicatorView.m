...
- (void)updateLayerUI {
    ...
    self.layer4.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
    self.layer4.mask = [self maskLayerForLayer4];
}
...
- (UIImageView *)pointerView {
    if (!_pointerView) {
       ...
       // _pointerView.frame =  CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
       // _pointerView.contentMode = UIViewContentModeScaleAspectFill;
       // _pointerView.center = CGPointMake(self.circleCenter.x, self.circleCenter.y);
        //_pointerView.layer.anchorPoint = CGPointMake(0.5f, 0.8f); // 锚点
        
        _pointerView.frame =  CGRectMake(self.bounds.size.width/2-10, self.bounds.size.height/2-10, 20, 60);
        _pointerView.contentMode = UIViewContentModeScaleAspectFill;
        _pointerView.layer.anchorPoint = CGPointMake(0.5f, 0.8f); // 锚点
        
        
        //_pointerView.transform = CGAffineTransformMakeRotation(-(M_PI/2 + M_PI_4));
        _pointerView.transform = CGAffineTransformMakeRotation(-1.6);
...
- (void)touchAction:(UILongPressGestureRecognizer *)pan {
 ...
 //   CGFloat angle = ((CGFloat)_indicatorValue/(self.maxValue - self.minValue) - 0.5) *  (360 - self.openAngle);
    //指针转动角度
...
- (void)updateUI {
  ...
  //  NSRange range = [valueString rangeOfString:@"°C"];
    [stringM addAttribute:NSFontAttributeName value:DefaultFont(self.centerValueLabelFontSize) range:NSMakeRange(0, stringM.length )]; // 设置字体字号
...
- (void)setDataThatIsRelatedToFrameToCustomRatio {
    ...
    self.innerAnnulusScaleLabelFontSize = 10.0 * ApplyRatio(self.innerAnnulusScaleLabelFontSize_ratio);
...
- (CAShapeLayer *)maskLayerForLayer1 {
    ...
    for (int i = 0; i < self.outerAnnulusLineCountToShow; i++) {
        ...
        CGPoint cee = CGPointMake(self.circleCenter.x, self.circleCenter.y);
       // self.circleCenter = CGPointMake(circleCenterX, circleCenterY);
        
        NSArray *rectanglePointArray = [self calculateFourKeyPointForRectangleWithCircleCenter:cee innerCircleRadius:self.outerAnnulusInnerCircleRadius rectangleWidht:self.outerAnnulusRectangleWidht rectangleHeight:self.outerAnnulusRectangleHeight angle:angleTemp];
        
      //  NSArray *rectanglePointArray = [self calculateFourKeyPointForRectangleWithCircleCenter:self.circleCenter innerCircleRadius:self.innerAnnulusInnerCircleRadius rectangleWidht:self.innerAnnulusRectangleWidht rectangleHeight:self.innerAnnulusRectangleHeight angle:angleTemp];
        
        
     //   [self calculateFourKeyPointForRectangleWithCircleCenter:self.circleCenter innerCircleRadius:self.innerAnnulusInnerCircleRadius rectangleWidht:self.innerAnnulusRectangleWidht rectangleHeight:self.innerAnnulusRectangleHeight angle:angleTemp];
...
- (float)valueForTick:(int)tick
{
   ...
   CGFloat divisionValue = (self.maxValue - self.minValue) / 8.0;
   return tick * (divisionValue / 5.0) + self.minValue;
}
...
- (CAShapeLayer *)maskLayerForLayer3 {
   ...
   /* for (NSInteger i = 0; i<= divide; i++) {
        ...
        UIBezierPath *dotPath = [UIBezierPath bezierPathWithArcCenter:_circleCenter radius:self.dotRadius startAngle:startAngel endAngle:endAngel clockwise:YES];
      ...
      //  UIBezierPath *tickPath = [UIBezierPath bezierPathWithArcCenter:LuCenter radius:self.scaleRadius startAngle:startAngel endAngle:endAngel clockwise:YES];
        ...
        perLayer.path = dotPath.CGPath;
        [self.layer  addSublayer:perLayer];
    
    }*/
    
    // 1.添加矩形  innerAnnulusValueToShowArray
   ...
   for (int i = 0; i < self.innerAnnulusLineCountToShow; i++) {
        ...
        if (!isScaleLine) {
            ...
            CGFloat innerCircleRadius = self.innerAnnulusInnerCircleRadius - (self.innerAnnulusScaleRectangleHeight - self.innerAnnulusRectangleHeight)-5;
    ...
    for (int i = 0; i < self.innerAnnulusValueToShowArray.count; i++) {
        ...
        scaleLabel.text = [NSString stringWithFormat:@"%@", self.innerAnnulusValueToShowArray[i]];
        ...
        CGFloat tempRadian = AngleToRadian(360 - angle);
        CGFloat minus = self.innerAnnulusScaleRectangleHeight - self.innerAnnulusRectangleHeight+5;
...
/// layer4 的 maskLayer
- (CAShapeLayer *)maskLayerForLayer4 {
    
    CAShapeLayer * maskLayer= [CAShapeLayer layer];
    maskLayer.frame = CGRectMake(0, 0, self.layer4.bounds.size.width, self.layer4.bounds.size.height);
    maskLayer.lineWidth   = 50;
    
    UIBezierPath *basePath = [UIBezierPath bezierPath];
    
    
    CGFloat scaleDivisions = 12.0;
    CGFloat scaleSubdivisions = 3.0;
    
    NSInteger fromLineNumber = [self lineNumberWithIndicatorValue:self.minValue];
    NSInteger toLineNumber = [self lineNumberWithIndicatorValue:self.maxValue];
    int minus = (int)(toLineNumber - fromLineNumber);
    
  //  int totalTicks = scaleDivisions * scaleSubdivisions + 1;
...
//
  ...
  //  self.innerAnnulusAngleEveryLine = (360.0 - self.openAngle) / space;
   ...
   // 180/36= 5
    CGFloat AngleEveryLine = (360.0 - self.openAngle) / space;
    
    for (int i = 0; i <= 36; i++) {

        
        CGFloat angleTemp = self.startAngle + i * 5;
        
       // CGPoint cc = CGPointMake(self.circleCenter.x-30, self.circleCenter.y-30);
        
        
       // self.outerAnnulusInnerCircleRadius = 225.0 * 0.5 * ApplyRatio(self.outerAnnulusInnerCircleRadius_ratio);
       // self.innerAnnulusInnerCircleRadius = 192.0 * 0.5 * ApplyRatio(self.innerAnnulusInnerCircleRadius_ratio);
        
        CGFloat  CircleRadius=  140.0 * 0.5 * ApplyRatio(self.innerAnnulusInnerCircleRadius_ratio);
        int hh = 5.0;
        if (i % 3 == 0 ) {
            hh= 10;
        }
        NSArray *rectanglePointArray = [self calculateFourKeyPointForRectangleWithCircleCenter:self.circleCenter innerCircleRadius:CircleRadius rectangleWidht:self.innerAnnulusRectangleWidht rectangleHeight:hh angle:angleTemp];
...
- (void)setIndicatorValue:(NSInteger)indicatorValue animated:(BOOL)animated {
  ...
  //  CGFloat toValue = self.minValue;
   ...
   // }
    ...
    CGFloat angle = ((CGFloat)_indicatorValue/(self.maxValue - self.minValue)  - 0.5) * AngleToRadian(360 - self.openAngle);
